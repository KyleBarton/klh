...
** 2024-04-02 
So a command like "display::attach_buffer" shows pretty clearly that I
need command to include optional content. It needs to be generic but
serializable. Need to figure that out.
Wait, no I'm misremembering. MessageContent already exists for this.

Ok, so how would a "cross-plugin" thing work? An example:
buffer::update_content -> this updates the content of a given buffer
/However/, it might also update the viewable content of the associated window.

So:
- display has to listen for buffer::update_content
- display /also/ needs to read the content

So... buffer plugins needs to have a public "Buffer Requests" module I
think? Because dependent plugins like display need both the
MessageType ID and the Message Content.

But there has to be some sort of public MessageTypeID concept (maybe
just constant string-likes?), so that display can tell the dispatch
that it wants to subscribe to these particular messages.
** 2024-04-09
Ok, making some progress:

- Modify the main.rs to allow you to specify window & buffer names - DONE
- Enforce uniqueness on those names - DONE
- Start on buffer-to-window association

Oooh, created an interesting problem. When I attempt to create a
buffer name that already exists, subsequent bl commands fail?
It's like it knocks the whole plugin out of commission. That's interesting.
- Fixed by actually handling the error instead of panicking in plugin_channel.

** 2024-11-16

Ok, I think the point of my display_plugin_poc gets answered with this
commit. Basically, I needed an answer to the questions:
- How will the display and buffer plugins interface?
- How will plugins talk to each other more generally?

While the latter still has some ambiguity, I have a working approach
for the former. Display will be the "front end" and accept command
inputs. This will then send downstream commands to, e.g. the buffer
plugin. I think this is appropriate but makes the design assumption
that commands are sticky to a window, frame, or other visual context.
This is something I should consider long-term. Gets me off the ground
for now though.

One thing I still need to figure out is how to make plugin commands
discoverable. Also, possibly the display plugin should just become the
"command plugin" with implementations to talk to display and buffer
downstream? Whatever, need to move forward with something so I can
iterate.

Ok, I think there's no way around it: I need to create an "event"
message type to which clients can subscribe, sent _from_ plugins.

Tasks to make this work:
- Create a "subscription" flow in the client
- Copy the plugin_registrar logic for client subscribers (e.g. subscriber_registrar)
