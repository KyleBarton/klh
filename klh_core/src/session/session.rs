...
impl Session {
    ...
    pub fn register_plugin(&mut self, plugin: Box<dyn Plugin + Send>) {
        ...
        self.plugin_registrar
            .register_message_types_for_plugin(message_types, channel.get_transmitter());
    ...
    pub async fn run(&mut self) -> Result<(), SessionError> {
        ...
        match self.dispatch.take() {
            ...
            Some(mut dispatch) => {
                ...
                tokio::spawn(async move { dispatch.start(registrar_copy).await.unwrap() });
                Ok(())
            }
        }
    }
...
mod session_tests {
    ...
    async fn should_send_message_to_registered_plugin(mut default_session: Session) {
        ...
        let deserialized_response: String = response
            .deserialize()
    ...
    async fn should_handle_unknown_message(mut default_session: Session) {
        ...
        let mut unknown_request =
            Request::from_message_type(MessageType::command_from_str("unknown").unwrap());
    ...
    async fn should_not_be_able_to_start_started_session(mut default_session: Session) {
        ...
        assert_eq!(
            unsuccessful_run.err(),
            Some(SessionError::SessionAlreadyStarted)
        );
    }
