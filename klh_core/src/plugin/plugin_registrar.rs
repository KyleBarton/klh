...
use crate::messaging::{
    CommandMessage, EventMessage, Message, MessageContent, MessageError, MessageType,
};
        ...
        for message_type in message_types.iter() {
            ...
            match message_type {
                MessageType::Command(_) => {
                    self.command_plugin_map
                        .insert(*message_type, transmitter.clone());
                }
                MessageType::Event(_) => match self.event_plugin_map.get_mut(message_type) {
                    Some(transmitters) => {
        ...
        match self.event_plugin_map.get(&message.get_message_type()) {
            Some(listeners) => {
                for listener in listeners {
                    listener
                        .send_message(Message::Event(message.clone()))
                        .await
                        .unwrap();
                }
            }
    ...
    async fn send_command_to_plugin(&self, mut message: CommandMessage) {
        ...
        match self.command_plugin_map.get(&message.get_message_type()) {
            ...
            None => {
                ...
                if let Err(message_error) = message
                    .get_responder()
                    .expect("Should have a responder")
                    .respond(MessageContent::from_content(
                        MessageError::MessageTypeNotFound,
                    ))
                {
                    debug!(
