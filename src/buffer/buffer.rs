...
impl LineBuffer {
  ...
  fn delete(&mut self) -> Result<(), String> {
    ...
    if self.point.offset == 0 {
      self.point = if self.point.line < 1 {
        Coordinate { offset: 0, line: 0 }
      } else {
...
impl Buffer for LineBuffer {
  ...
  fn move_current_location(&mut self, amount: i64) -> Result<Coordinate, String> {
    if amount < 0 {
      ...
      for _n in 0..amount {
        self.point.offset = if self.point.offset == self.lines[self.point.line].len() {
